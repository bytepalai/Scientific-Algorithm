...
class Faceverification(Resource):
    ...
    def post(self): # Post route, will need better names
        #global model #To be reviewed
        r = request
        # convert string of image data to uint8
        img1 = cv2.imdecode(np.fromstring(request.files['image1'].read(), np.uint8), cv2.IMREAD_UNCHANGED)
        img2 = cv2.imdecode(np.fromstring(request.files['image2'].read(), np.uint8), cv2.IMREAD_UNCHANGED)
        # Need to think about asynchronicity in case i have many requests at the same time
        # Save image
        cv2.imwrite('./image1.jpg', img1) # Do i need to always save and read, maybe that might waste some time, also that might be customer data so maybe i shouldn't save or maybe i should
        ...
        output = recognition_model('./image1.jpg','./image2.jpg')
